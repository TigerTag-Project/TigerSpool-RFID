...
| **The UI faces carry Latin-1, Latin Extended-A and only the Chinese the source uses.** | Accents draw. A Chinese string with a character not yet in the source draws as a blank box until the faces are regenerated (`scripts/make-ui-font.sh`). `check-ui-fonts.py` catches both cases; see the font row in [CLAUDE.md](CLAUDE.md#hardware-facts-that-bite). |
