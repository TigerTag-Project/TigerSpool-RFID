...
void buildScreen() {
    ...
    // 240 px: at 16 "Imprimantes" ends at x=114 and the refresh glyph starts
    ...
    // Ask the account again, now. The list refreshes on its own every five
    // minutes, which is right for a box on a shelf and useless to somebody who
    // has just added a printer in Tiger Studio and is standing in front of the
    // device waiting for it. Its hit area is a full 44 px tall like the gear's:
    // the glyph is small, the target is not.
    lv_obj_t* reload = lv_btn_create(header);
    lv_obj_remove_style_all(reload);
    // 32 wide, not 52. Four controls and a title share 240 px, and this is the
    // one that can give: at 40 it overlapped the last letter of "Imprimantes".
    // Still 44 px tall, which is the dimension a thumb actually misses on.
    ...
    // Four controls and a title share 240 px, so these offsets are measured
    // against a capture rather than chosen: the title's last letter ends at
    // x=110 and the refresh glyph starts at 118. Widen anything here and they
    // touch - which they did at the first attempt.
...
void show(const PrinterCfg* printers, int count,
    ...
    // The reload glyph is the progress indicator too. A button that does
